...
| パス | 用途 |
|:-----|:-----|
| `10_スキル/` | スキルマップ・職能別（`10_common/` `20_CS業務/` `30_営業/` 等） |
| `20_プレイブック/` | 業務フロー・`20_プロセス/` `30_テンプレート/` |
| `30_プロジェクト/` | 本業プロジェクト |
| `40_マネジメント/` | マネジメント系 |
...
| パス | 用途 |
|:-----|:-----|
| `RINGBELL/` | 副業（婚活 CS）の索引・業務設計・`50_AI自動化/`（棚: `00_`〜`30_` は `50_AI自動化/00_AI自動化_フォルダ設計.md`） |
...
| `02_AI導入コンサルテンプレートフォルダ/` | AI 導入コンサル向け**テンプレ正本**（`00`メタ・`01`司令塔・`02`マスター・`03`Executive） |
| `03_手法/` | 手法メモ（**`ツール_作業内容.md`** 形式）。読書・ファクトチェック・MCP・NotebookLM・Claude Code 等 |
| `04_学習/` | 講座・参照メモ（**`ClaudeCode_`／`AI_`／`Obsidian_`＋内容.md** など）。**`今後やること.md`（実行ハブ）** は固定名 |
| `90_SNSノウハウ/` | note・X 等 **SNS／コンテンツ配信**の型・検証メモ（`note_` など） |
| `80_実験ログ/` | 短期実験・宿題 |
| `99_アーカイブ/` | 旧版・退避 |
...
| `03_RSS/` | RSS 取り込みの受け皿・`_購読リスト.md`・`CLAUDE.md`（設計は `04_AI/04_学習/10_リサーチ/ObsidianRSS_LINE取り込み設計.md`） |
