...
> **出典・位置づけ:** チャエン資料のスキル分類を整理した**参照用カタログ**（コマンド名・用途の早見）。**実体のスキルファイルは配布元・`.claude/skills/` に別途必要。** 全文脈・運用・MCPは [[10_リサーチ/ClaudeCodeチャエン_スキルMCP運用]] を正とする。  
> **Cursor** の `/コマンド` や Agent Skills とは**別物**（[[../03_手法/20_ClaudeCode/20_ClaudeCode設計_スキル設計要点]]）。
