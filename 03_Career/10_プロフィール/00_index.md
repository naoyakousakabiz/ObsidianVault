...
**流れ:** 正本を更新 → `20_マスタ/` で提出文 → 企業別は `40_転職活動/.../companies/`（転職オペの地図は `40_転職活動/00_index.md`）。戦略に採用する結論だけ `30_キャリア戦略/20_planning.md` へ。市場一般論の束ねは `30_キャリア戦略/30_synthesis/`（戦略フォルダの地図は `30_キャリア戦略/00_index.md`）。
...
| 観測できた事実・自分の行動 | 市場一般論だけ（→ `30_synthesis/`） |
