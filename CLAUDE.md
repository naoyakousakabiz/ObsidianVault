...
- **主対象:** 副業 RINGBELL（婚活CS）の実務。設計・施策の正本は `02_Business/RINGBELL/`（`50_AI自動化/` を含む）。
...
20_Input        インプット系（自動同期・手動逃がし含む）
  ...
  └ 03_RSS/     RSS 取り込み（購読リスト・設計は `04_AI/04_学習/10_リサーチ/ObsidianRSS_LINE取り込み設計.md`）
...
**最短の開き方:** ツール運用の棚卸し → `04_AI/01_ツール運用/00_索引.md`｜**今後やること（30日×Phase×継続）** → `04_AI/04_学習/01_実行計画.md`｜ルーティン → `04_AI/01_ツール運用/10_運用ルール.md`｜副業 RINGBELL → `02_Business/RINGBELL/00_索引.md`
...
- 機微を Vault に書き込む可否の**枠組み**は `04_AI/01_ツール運用/02_フェーズとガバナンス.md` を正とする（本節と矛盾したらそちら優先）。
